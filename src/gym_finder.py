...
class GymFinder:
    ...
    def fuzzy_match_gyms(self, yelp_gyms, google_gyms, confidence_threshold=0.35):
        ...
        for yelp_gym in yelp_gyms:
            ...
            for idx, google_gym in enumerate(google_gyms):
                ...
                if yelp_address_norm and google_address_norm:
                    ...
                    if yelp_coords[0] and google_coords.get("lat"):
                        distance = self.calculate_distance(
                            yelp_coords[0], yelp_coords[1], google_coords["lat"], google_coords["lng"]
                        )
                ...
                if "business_hours" in google_gym:
                    hours_similarity = self.compare_business_hours(yelp_gym.get("hours"), google_gym["business_hours"])
                ...
                if "types" in google_gym:
                    ...
                    semantic_similarity = self.semantic_category_mapping("gyms,fitness", google_gym["types"])
                ...
                if "price" in yelp_gym and "price" in google_gym:
                    price_similarity = self.compare_price_ranges(yelp_gym["price"], google_gym.get("price_level"))
                ...
                review_bonus = self.compare_review_counts(yelp_gym.get("review_count", 0), google_gym.get("review_count", 0))
                confidence += review_bonus * 0.5  # Reduced impact

                # Website quality assessment (reduced)
                website_quality_bonus = self.assess_website_quality(yelp_gym.get("url", ""), google_gym.get("website", ""))
                confidence += website_quality_bonus * 0.5  # Reduced impact

                # Enhanced phone number intelligence (reduced)
                phone_intelligence_bonus = self.enhanced_phone_matching(yelp_gym.get("phone", ""), google_gym.get("phone", ""))
        ...
        avg_confidence = sum(
            gym.get("match_confidence", 0) for gym in merged_gyms if gym.get("match_confidence", 0) > 0
        ) / max(merged_count, 1)
    ...
    def export_to_json(self, gyms, zipcode, filename=None):
        ...
        export_data = {
            "search_info": {"zipcode": zipcode, "timestamp": datetime.now().isoformat(), "total_results": len(gyms)},
            "gyms": gyms,
        }
