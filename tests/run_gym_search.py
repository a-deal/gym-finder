...
def run_gym_search(zipcode, radius=10, export_format=None, use_google=True, quiet=False):
    ...
    if not lat or not lng:
        return {
            'error': f'Could not find coordinates for zipcode {zipcode}',
            'results': []
        }
    ...
    if not all_gyms:
        return {
            'error': 'No gyms found from any source',
            'results': []
        }
    ...
    avg_confidence = 0.0
    if merged_count > 0:
        avg_confidence = sum(gym.get('match_confidence', 0) for gym in all_gyms if gym.get('match_confidence', 0) > 0) / merged_count
