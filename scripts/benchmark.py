...
def comprehensive_benchmark():
    ...
    for radius in [1, 2, 5]:
        ...
        radius_tests[radius] = {"time": execution_time, "total_gyms": total_gyms}
    ...
    print(
        f"✅ Scalability: {min(radius_tests.values(), key=lambda x: x['time'])['time']:.2f}s - {max(radius_tests.values(), key=lambda x: x['time'])['time']:.2f}s"
    )
